...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`rand`](https://crates.io/crates/rand).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `rand` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `rand` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
