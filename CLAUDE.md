# Claude Code Project Instructions — rand-kotlin

## Project Overview

This is **rand-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rand`](https://crates.io/crates/rand).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rand/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rand>
